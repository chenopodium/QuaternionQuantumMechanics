...
public class Shell
{
	...
	*/
	public Shell(float r, float minRadius, float maxRadius,  GameObject shellObject, float pInfluence, int dropoffFunction) {
		...
		this.fraction = (float)Math.Min(1.0, Math.Max(0, (maxRadius - r) / delta));
		// minRadius should be 1
		float dr = r - minRadius;
		float f = 0.25f / pInfluence;
		if (dropoffFunction == GameManager.DROPOFF_WAVE) {
			// drop off wit
			this.fraction = 1.0f / (1.0f + dr * dr * f);
		}
		if (dropoffFunction == GameManager.DROPOFF_RSQUARED) {
			this.fraction = 1.0f /(1.0f +  dr*dr*f);			
		}
